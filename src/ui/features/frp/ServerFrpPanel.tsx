...
export function ServerFrpPanel(state: ServerFrpPanelState) {
  ...
  return (
    <section aria-labelledby="frp-server-heading" className="space-y-4 p-4 bg-white rounded shadow border">
      <h2 id="frp-server-heading" className="text-lg font-semibold">FRP Server Management</h2>
      ...
      <dl className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
        <div><dt className="font-medium">Panel URL</dt><dd>{config.panelUrl || "not configured"}</dd></div>
        <div><dt className="font-medium">RPC URL</dt><dd>{config.rpcUrl || "not configured"}</dd></div>
        <div><dt className="font-medium">Server address</dt><dd>{config.serverAddr || "not configured"}</dd></div>
        <div><dt className="font-medium">Bind port</dt><dd>{config.bindPort}</dd></div>
        <div><dt className="font-medium">Dashboard status</dt><dd>{config.dashboardEnabled ? "Dashboard enabled" : "Dashboard disabled"}</dd></div>
        <div><dt className="font-medium">Token reference</dt><dd>{maskFrpTokenRef(config.authTokenRef)}</dd></div>
      </dl>
      <form
      ...
      >
        ...
        <label className="space-y-1">
          <span className="font-medium">Bind port</span>
        ...
        <button type="submit" disabled={!state.saveConfig || !validation.ok || actionPending} className="px-3 py-2 rounded bg-blue-600 text-white disabled:bg-gray-300">Save server config</button>
      </form>
      <section aria-label="FRP client list" className="space-y-2">
        ...
        {state.status.client ? (
          ...
          <p className="text-sm text-gray-500">No connected desktop clients reported.</p>
        )}
      </section>
      <section aria-label="Desktop connection information" className="space-y-2">
        <h3 className="font-medium">Desktop connection information</h3>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <button type="button" disabled={!state.startFrp || !validation.ok || actionPending || state.status.running} onClick={() => void state.startFrp?.()} className="px-3 py-2 rounded bg-green-600 text-white disabled:bg-gray-300">Start FRP server</button>
        <button type="button" disabled={!state.stopFrp || actionPending || !state.status.running} onClick={() => void state.stopFrp?.()} className="px-3 py-2 rounded bg-red-600 text-white disabled:bg-gray-300">Stop FRP server</button>
