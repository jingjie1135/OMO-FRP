...
describe("install-config server deploy plan", () => {
  it("returns the server closure needed by automation", () => {
    ...
    expect(plan.commands.join("\n")).toContain("cp deploy/server/.env.example /opt/test-opencode-remote/.env")
    expect(plan.commands.join("\n")).toContain("editor /opt/test-opencode-remote/.env")
    expect(plan.commands.join("\n")).toContain("OPENCODE_REMOTE_INSTALL_ROOT=/opt/test-opencode-remote deploy/server/install.sh")
