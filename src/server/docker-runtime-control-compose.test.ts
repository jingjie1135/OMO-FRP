...
test("server compose exposes Docker control settings without enabling the socket by default", async () => {
  ...
  expect(compose).toContain("FRP_PANEL_COMPOSE_PROJECT")
  expect(compose).toContain("FRP_PANEL_COMPOSE_SERVICE")
  expect(compose).toContain("MANAGEMENT_API_SESSION_TOKEN")
  expect(compose).toContain("OPENCODE_REMOTE_STATE_ROOT")
  expect(compose).toContain("management-ui-state:/runtime")
...
test("env example keeps Docker container control disabled by default", async () => {
  ...
  expect(envExample).toContain("OPENCODE_REMOTE_STATE_ROOT=/runtime")
  ...
  expect(envExample).toContain("FRP_PANEL_COMPOSE_PROJECT=opencode-remote-platform")
  expect(envExample).toContain("FRP_PANEL_COMPOSE_SERVICE=frp-panel")
  expect(envExample).toContain("MANAGEMENT_API_SESSION_TOKEN=")
})
